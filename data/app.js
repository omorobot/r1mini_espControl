...
websocket.onmessage = function (e) {
  updateConnectionStatus("New Message"+e.data);
...
function updateConnectionStatus(msg) {
  var tb = document.getElementById("TEXTBOX_CONNECTION_STATUS");
  if(tb!=null) {
  	tb.value = msg;
  }
}

function sendToggleState (id, state) {
	var checked=0;
	if(state) {
		 checked=1;
	}
	var toggleState = '#'+id.toString()+','+checked;
	console.log('Toggle state:'+toggleState);
	//websocket.send(toggleState);
